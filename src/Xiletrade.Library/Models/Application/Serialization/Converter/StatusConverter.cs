...
public sealed class StatusConverter(SourceGenerationContext context) : JsonConverter<OnlineStatus>
{
    ...
    public override OnlineStatus Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            var obj = JsonSerializer.Deserialize(ref reader, _context.OnlineStatus);
    ...
    public override void Write(Utf8JsonWriter writer, OnlineStatus value, JsonSerializerOptions options)
    {
        if (value is not null)
        {
            ...
            JsonSerializer.Serialize(writer, value, _context.OnlineStatus);
        }
        else
        {
            writer.WriteNullValue();
