...
//TO TEST
internal sealed class ItemOptionNew
{
    ...
    private string Get(string key)
        => _options.TryGetValue(key, out var value) ? value : string.Empty;
    ...
    internal string[] Resolve => Get(Resources.Resources.General114_SanctumResolve)
    ...
    internal bool Update(ReadOnlySpan<char> data)
    {
        ...
        ref string valueRef = ref CollectionsMarshal.GetValueRefOrNullRef(_options, key);
        if (!Unsafe.IsNullRef(ref valueRef))
        {
            if (valueRef.Length is 0)
            {
                valueRef = valueSpan.ToString();
            }
            return true;
        }
        return false;
